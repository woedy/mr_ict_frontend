...
addToTimeline: (trackType, trackId, clip) => get().updateState((state) => {
  ...
  const startTime = clip.hasOwnProperty('startTime') && clip.startTime !== undefined
...
checkerboardClips: (trackType, trackId, clipsToAdd, startPosition = 0, gap = 0) => get().updateState((state) => {
  const trackIndex = state.timelineTracks[trackType].findIndex(t => t.id === trackId);
  if (trackIndex === -1) return state;
  
  // Work with a sorted copy of clips
  const sortedClips = [...clipsToAdd].sort((a, b) => {
    // If no startTime is specified, maintain original order
  ...
  const positionedClips = sortedClips.map(clip => {
    const newClip = {
      ...clip,
      id: clip.id || Math.random().toString(36).substr(2, 9),
      startTime: currentPosition
    };
    currentPosition += clip.duration + gap;
    return newClip;
  });
  ...
  updateClip: (trackType, trackId, clipIndex, updates) => get().updateState((state) => {
    const trackIndex = state.timelineTracks[trackType].findIndex(t => t.id === trackId);
    if (trackIndex === -1) return state;
  ...
  moveClipToTrack: (sourceType, sourceTrackId, clipIndex, targetType, targetTrackId, newStartTime) => get().updateState((state) => {
    // Find source track
    const sourceTrackIndex = state.timelineTracks[sourceType].findIndex(t => t.id === sourceTrackId);
    if (sourceTrackIndex === -1) return state;
    
    // Find target track
    const targetTrackIndex = state.timelineTracks[targetType].findIndex(t => t.id === targetTrackId);
    ...
    const updatedTargetClips = [...targetTrack.clips, clipToMove].sort((a, b) => a.startTime - b.startTime);
...
duplicateTrack: (trackType, trackId) => get().updateState((state) => {
  const trackIndex = state.timelineTracks[trackType].findIndex(t => t.id === trackId);
  if (trackIndex === -1) return state;
...
importTimeline: (jsonData) => get().updateState((state) => {
  try {
